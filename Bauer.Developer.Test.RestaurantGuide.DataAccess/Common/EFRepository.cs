...
namespace Bauer.Developer.Test.RestaurantGuide.DataAccess.Common
{
    public class EFRepository<TObject> : IBaseRepository<TObject> where TObject : class, IEntity
    {
        ...
        public virtual int UpdateExcludeSpecificProperties(TObject TObject, Action<DbContext, TObject> specificPropertiesUnmodified)
        {
            var entry = Context.Entry(TObject);
            DbSet.Attach(TObject);
            entry.State = EntityState.Modified;
            if (specificPropertiesUnmodified != null)
            {
                specificPropertiesUnmodified(Context, TObject);
            }
            if (!shareContext)
                return Context.SaveChanges();
            return 0;
        ...
        public virtual int UpdateSpecificProperties(TObject TObject, Action<DbContext, TObject> specificPropertiesModified)
        {
            var entry = Context.Entry(TObject);
            DbSet.Attach(TObject);
            if (specificPropertiesModified != null)
            {
                specificPropertiesModified(Context, TObject);
            }
            if (!shareContext)
            {
                return Context.SaveChanges();
